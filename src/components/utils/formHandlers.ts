...
export const handleOptionChange = (event: React.ChangeEvent<HTMLInputElement>, dispatch: React.Dispatch<QuestionnaireAction>) => {
...
export const handleChangeVoen = (event: React.ChangeEvent<HTMLInputElement>, dispatch: React.Dispatch<QuestionnaireAction>) => {
    const { value } = event.target;
    if (/^\d*$/.test(value) && value.length <= 10) {
        dispatch({ type: 'SET_VOEN', voen: value });
    }
};
export const handleChangeBirthDate = (dispatch: React.Dispatch<QuestionnaireAction>, e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch({ type: 'SET_BIRTH_DATE', birthDate: e.target.value });
};

export const handleChangeConfirmPassword = (dispatch: React.Dispatch<QuestionnaireAction>, e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch({ type: 'SET_CONFIRM_PASSWORD', confirmPassword: e.target.value });
};

export const handleChangePassword = (dispatch: React.Dispatch<QuestionnaireAction>, e: React.ChangeEvent<HTMLInputElement>) => {
...
export const handleChangeInBusinessPhoneNum = (
    ...
    if (digits.length <= 9) {
        ...
        dispatch({ type: 'SET_BUSINESS_PHONE_NUMBER', businessPhoneNumber: formattedInput });
