...
import { getDayDetails, getNumberOfDays, getDateStringFromTimestamp } from '../utils/dateUtils';
...
export default class MyDatePicker extends Component<MyDatePickerProps, MyDatePickerState> {
    ...
    addBackDrop = (e: MouseEvent) => {
        ...
        if (
            ...
            !ReactDOM.findDOMNode(this)?.contains(e.target as Node) &&
            !ReactDOM.findDOMNode(this.inputRef.current)?.contains(e.target as Node)
        ) {
    ...
    setDateToInput = (timestamp: number) => {
        if (this.inputRef.current) {
            ...
            if (this.props.dispatch) this.props.dispatch({ type: 'SET_BIRTH_DATE', birthDate: dateString });
        }
    };
    ...
    render() {
        const { showDatePicker, month, year, monthDetails, selectedDay } = this.state;

        return (
            <div className={`${styles.MyDatePicker} `}>
                <div   className={`${styles.formControll} ${styles.formControllData} `}>
                    <div className={styles.formInput} onClick={() => this.showDatePicker(true)}>
                            <input className={`${styles.formInput} ${this.props.state?.errors.birthDate ? styles.error : ''}`} type="date" onChange={this.updateDateFromInput} ref={this.inputRef}/>
                        <label className={styles.required}
                               htmlFor="date">Date
                            of
                            birth</label>
                    </div>
                    <p className={styles.errorText}>{this.props.state?.errors.birthDate}</p>
                </div>

                {showDatePicker && (
                    <div className={styles['mdp-container']}>
                    <div className={styles['mdpc-head']}>
                            <div className={styles['mdpch-button']} onClick={() => this.setYear(-1)}>
                                <div className={styles['mdpchb-inner']}>
                                    <span className={styles['mdpchbi-left-arrows']}></span>
                                </div>
                            </div>
                            <div className={styles['mdpch-button']} onClick={() => this.setMonth(-1)}>
                                <div className={styles['mdpchb-inner']}>
